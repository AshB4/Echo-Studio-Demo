...
The Mac is the workstation for editing, product prep, content prep, and asset prep.
The HP is the poster/workhorse for live queue processing and platform automation.

Do not rewrite, update, refactor, or "improve" existing working code unless the user explicitly asks for code changes. When the user asks for queue cleanup, posting reconciliation, asset sync, or runtime operations, treat that as runtime state work, not permission to change source code.
